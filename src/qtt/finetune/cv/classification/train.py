...
from .custom_timm import CheckpointSaver, create_loader
...
from .utils.finetuning_utils import prepare_model_for_finetuning
...
def build_parser():
    ...
    group.add_argument("--local_rank", "--local-rank", default=0, type=int)
    parser.add_argument(
        "--device_modules",
        "--device-modules",
        default=None,
    ...
    group.add_argument(
        "--ra_num_magnitude_bins",
        "--ra-num-magnitude-bins",
        type=int,
        default=31,
    ...
    ####################################################################################
    group = parser.add_argument_group("Finetuning parameters")
    group.add_argument(
        "--pct_to_freeze",
        "--pct-to-freeze",
        type=float,
...
def main(args: argparse.Namespace, args_text: str) -> None:
    ...
    return_features = args.bss_reg or args.delta_reg or args.cotuning_reg
    return_source_output = args.delta_reg or args.cotuning_reg
    ...
    # relationship for co-tuning
    relationship = None
    if args.cotuning_reg > 0.0:
        fsuf = args.dataset.lower() if args.dataset else os.path.basename(args.data_dir)
        file_name = f"{args.model}_{fsuf}.npy"
        output_dir = utils.get_outdir(os.path.join(args.output, "relationships"))
        rel_loader = create_loader(
...
def train_one_epoch(
    ...
    if utils.is_primary(args):
        logger.info(
            f"*** TRAIN: {epoch}  "
            f"TotalTime: {update_time_m.sum:.2f}s  "
    ...
    return OrderedDict([("loss", losses_m.avg), ("time", update_time_m.sum)])
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if utils.is_primary(args) and (
                not batch_idx % args.log_interval or last_batch
            ):
    ...
    if utils.is_primary(args):
        logger.info(
            f"*** VAL:  Acc@1: {top1_m.avg:.2f}  "
            f"Acc@5: {top5_m.avg:.2f}  "
            f"Loss: {losses_m.avg:.3f}  "
            f"Time: {batch_time_m.sum:.2f}"
        )
