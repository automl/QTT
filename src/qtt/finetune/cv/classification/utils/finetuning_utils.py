...
class IntermediateLayerGetter(nn.Module):
    ...
    def __call__(self, *args, **kwargs):
        ...
        # TODO: test if the gradients are computed through the features for BSS
...
def get_layers(model, num_classes, device, freezable_thd=0.0, change_head=False):
    if isinstance(model, models.edgenext.EdgeNeXt):
    ...
    elif isinstance(model, models.deit.VisionTransformer) or isinstance(
        ...
        isinstance(model, models.swin_transformer_v2.SwinTransformerV2)
        or isinstance(model, models.swin_transformer_v2_cr.SwinTransformerV2Cr)
        or isinstance(model, models.swin_transformer.SwinTransformer)
    ):
        source_head = nn.Sequential(copy.deepcopy(model.head))
        if change_head:
            num_ftrs = model.head.in_features
...
def get_layers_by_module_children(
    model, num_classes, device, freezable_thd=0.0, change_head=False
):
    if isinstance(model, models.edgenext.EdgeNeXt):
        source_head = nn.Sequential(
            copy.deepcopy(model.head.global_pool),
...
def prepare_model_for_finetuning(
    ...
    pct_to_freeze: float = 0.0,
    freezable_thd: float = 0.0,
