...
class SPRegularization(nn.Module):
    ...
    def __init__(
        self, model: nn.Module, head_name, regularization_weight: float = 0.01
    ):
        ...
        for name, param in model.named_parameters():
            if (
                not name.startswith(self.head_name)
            ) and param.requires_grad:  # it might not work for every model
        ...
        for name, param in self.target_model.named_parameters():
            if name in self.source_weight:
                ...
                output += (
                    ...
                    * torch.norm(param - self.source_weight[name]) ** 2
                )
        return output
...
class BehavioralRegularization(nn.Module):
    ...
    def forward(self, layer_outputs_source, layer_outputs_target):
        output = self.regularization_weight * (
            torch.norm(layer_outputs_target - layer_outputs_source.detach()) ** 2
        )
        return output
...
class Relationship(object):
    ...
    def __init__(self, data_loader, classifier, device, cache=None):
        ...
        if cache is None or not os.path.exists(cache):
            ...
            self.relationship = self.get_category_relationship(
                source_predictions, target_labels
            )
    ...
    def collect_labels(self):
        ...
        logger.debug("Collecting labels to calculate relationship")
        source_predictions = []
        target_labels = []
...
def convert_to_stoch_norm(module, p=0.5):
    ...
    mod = module
    for pth_module, stoch_module in zip(
        [
            torch.nn.modules.batchnorm.BatchNorm1d,
            torch.nn.modules.batchnorm.BatchNorm2d,
            torch.nn.modules.batchnorm.BatchNorm3d,
        ],
        [StochNorm1d, StochNorm2d, StochNorm3d],
    ):
